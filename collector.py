...
BLACKLIST_STOCKS = {
    ...
    '하나증권', '메리츠증권', '메리츠금융지주', 'KB증권', 'KB금융', '대신증권', '한화투자증권', '유안타증권',
...
# 기본 노이즈 필터
NOISE_STOCKS = {'전방', '대상', '지구', '신세계', '가스', '전선', '화재', '국보', '백산', '나노', '레이', '보물', '유진', '대원', '효성', '선진', '동방', '서원', '대성', '우진', '한화', '두산', '삼성', '현대', 'SK', 'LG', '부국', '국내', '미국', '중국', '일본', '해외', '시장', '금융', '증권', '투자', '매수', '매도', '추천', '비중', '전망', '분석', '이슈', '테마', '섹터', '코스피', '코스닥', '지수', '상한가', '하한가', '베뉴지', '홀딩스', '그룹', '우', '채널', '입장', '보기', '매매', '공부', '참여', '문의', '상담', '종목', '주식', '코리아', '서울', '부산', '대구', '인천', '광주', '대전', '울산', '세종', '경기', '강원', '충북', '충남', '전북', '전남', '경북', '경남', '제주'}

# [핵심] 키워드 필터링 대폭 강화 (쓰레기 단어 제거)
STOP_KEYWORDS = {
...
def save_db(stock_map, kiwi):
    ...
    for rank, (s, ctx) in enumerate(sorted_stocks, 1):
        try:
            ...
            # [키워드 정제 강화]
            blob = " ".join(ctx)
            # 1글자짜리 명사는 무조건 버림 (예: '수', '급', '전')
            kws = [t.form for t in kiwi.tokenize(blob[:1000]) if t.tag.startswith('NN') and len(t.form) >= 2 and t.form!=s]
            valid_kws = [w for w in kws if w not in STOP_KEYWORDS and w not in ABSOLUTE_IGNORE and w not in BLACKLIST_STOCKS]
            
            # 가장 많이 나온 키워드 3개 추출
            reason = ", ".join([w for w, _ in Counter(valid_kws).most_common(3)])
            
            # 키워드가 비어있으면 뉴스 내용에서라도 뽑아봄
            if not reason:
                reason = "뉴스참조"
            ...
            async for m in client.iter_messages(ent, limit=200):
                if m.text and len(m.text) > 2:
                    ...
                    for s in stock_names:
                        if s in m.text:
                            if s in BLACKLIST_STOCKS: continue
                            if '증권' in s or '투자' in s or '스팩' in s: continue
